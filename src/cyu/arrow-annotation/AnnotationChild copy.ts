...
// 			const { blendEl, decorEl } = this.createLabel(rule.label)
...
// 	// 	}
//
// 	private createLabel(markdownText: string): {
// 		/** 混合文本图层：只负责渲染 MD 文本，用于 mix-blend-mode */
// 		blendEl: HTMLElement
// 		/** 装饰/定位图层：负责画圈、波浪线、接受 transform 定位等 */
// 		decorEl: HTMLElement
// 	} {
// 		// 1. 创建混合图层（只存文本内容）
// 		const blendEl = document.createElement('div')
// 		blendEl.classList.add('annotation-label-blend') // 新的类名，专做混合
//
// 		// 渲染 md 语法到混合图层
// 		MarkdownRenderer.render(this.app, markdownText, blendEl, this.ctx.sourcePath, this)
//
// 		// 展开并清理 <p> 标签（保持你原本的逻辑）
// 		const p = blendEl.querySelector('p')
// 		if (p && p.parentElement === blendEl && blendEl.children.length === 1) {
// 			while (p.firstChild) blendEl.appendChild(p.firstChild)
// 			p.remove()
// 		}
//
// 		// 分类挂属性（依然挂在混合图层上，或者两边都挂，看你 CSS 怎么写）
// 		const type = this.classifyText(blendEl)
// 		blendEl.setAttr('annotationType', type)
//
// 		// 2. 创建独立的装饰图层（用于后续画圈、波浪线、做 transform）
// 		const decorEl = document.createElement('div')
// 		decorEl.classList.add('annotation-label-decor') // 专做装饰
// 		decorEl.setAttr('annotationType', type)
//
// 		return { blendEl, decorEl }
// 	}
//
